...
def shuffle_dataframe_and_series(dataframe, series):
    ...
    shuffled_dataframe = dataframe.reindex(shuffled_index).reset_index(drop=True)
...
def correlation_coef(x: np.ndarray, y: np.ndarray):
    ...
    if has_ties:
        ...
        return 1 - n / 2 * (np.sum(np.abs(ranks[:-1] - ranks[1:]))) / (np.sum(inverse_ranks * (n - inverse_ranks)))
    else:
