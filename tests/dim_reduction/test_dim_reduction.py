...
def test_compute_projection():  # not ok
    ...
    np.testing.assert_array_equal(
        compute_projection(X, y, 1, 2, callback).index, X.index)
...
def test_dim_reduction(
    ...
    for dim_method in [1, 2, 3]:
        ...
        cpt_proj_2D = compute_projection(X, y, dim_method, 2, callback,
                                         **params)
        assert cpt_proj_2D.shape == (len(X), 2)
        assert X.index.equals(cpt_proj_2D.index)
        assert callback.calls[-1][0][0] == 100

        cpt_proj_3D = compute_projection(X, y, dim_method, 3, callback,
                                         **params)
