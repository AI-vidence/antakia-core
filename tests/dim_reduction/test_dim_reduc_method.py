...
class TestDimReducMethod(TestCase):
    ...
    def test_init(self):
        ...
        drm = DimReducMethod(1, None, 2, self.X, progress_updated=dummy_progress)
        ...
        drm1 = DimReducMethod(2, None, 2, self.X, progress_updated=callback.call)
        ...
        drm2 = DimReducMethod(-1, None, 2, self.X, progress_updated=callback.call)
    ...
    def test_dimreduc_methods_as_list(self):
        assert DimReducMethod.dimreduc_methods_as_list() == list(range(1, len(DimReducMethod.dim_reduc_methods) + 1))
    ...
    def test_compute(self):  # ok rajouter test sur publish_progress
        drm = DimReducMethod(1, PCA, 2, self.X, default_parameters={'n_components': 2})
        drm.compute()
        assert drm.default_parameters == {'n_components': 2}

        drm1 = DimReducMethod(1, TSNE, 2, self.X, default_parameters={'n_components': 2})
        drm1.compute()


    def test_scale_value_space(self):
        np.random.seed(10)
        self.X = pd.DataFrame(np.random.randint(0, 100, size=(6, 3)), columns=list('ABC'))
        y = self.X.sum(axis=1)
        ...
        assert np.round(DimReducMethod.scale_value_space(self.X, y)[::], 6).equals(expected)